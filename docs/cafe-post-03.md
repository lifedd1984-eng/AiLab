...
- 복붙용 텍스트: `docs/cafe-post-03-paste.txt` (마크다운 기호 제거, 카페 에디터용)
...
위 화면들은 제가 쓰려고 만든 ELS 레이더(elsrader.site)라는 도구입니다. 원래 제 175건 관리하려고 만든 거라 투박한데, 궁금하신 분들 있을 것 같아 적어둡니다.
...
3. **본문 끝에 `elsrader.site` 를 한 번 넣었습니다 (확정).** 하이퍼링크로 걸지 말고 글자 그대로 두는 것을 권장합니다. 링크로 걸면 카페 자동 필터에 홍보 글로 잡히는 경우가 있습니다. 게시 후 제재가 걱정되면 이 한 문장만 빼고 올린 뒤 댓글로 안내하는 방법도 있습니다.
4. **1·2편 참조를 전부 제거했습니다.** 원래 초안은 "지난 글에서 말씀드렸듯이"가 네 번 나왔는데, 이 글은 1·2편을 올린 곳과 다른 카페용이라 독자가 그 글들을 본 적이 없습니다. 해당 부분을 전부 이 글 안에서 설명이 끝나도록 다시 썼습니다.
5. **랜딩 화면은 넣지 않았습니다.** 베타 배지와 가입 버튼이 있어 홍보 글로 분류될 위험이 큽니다.
